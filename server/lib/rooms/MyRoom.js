...
class MyRoom extends colyseus_1.Room {
    constructor() {
        ...
        this.STAGE_CREDITS = 500; // Starting credit budget
        this.interactableSpawnCooldown = 0;
        // Beacon system
        this.beaconId = null;
        this.BEACON_ACTIVATION_TIME = 90; // 90 seconds
        this.BOSS_SPAWN_DELAY = 2000; // 2 seconds after beacon activation
        // Time-based difficulty system
        this.DIFFICULTY_INTERVAL = 60; // 60 seconds per difficulty level
        ...
        this.GRID_SIZE = 100; // 100px grid cells
        ...
        this.xpEntitiesArray = [];
        this.lastEntityCount = { players: 0, enemies: 0, projectiles: 0, xpEntities: 0 };
        // Performance optimization: Distance calculation cache
        this.distanceCache = new Map();
        this.hitCounters = new Map();
        this.lastCacheClear = 0;
        // Map to store debounce timers for each player
        this.statCalcDebounceTimers = new Map();
        this.STAT_CALC_DEBOUNCE_MS = 50; // Debounce time in milliseconds
        this.noop = (...args) => { };
        ...
        this.update = (deltaTime) => {
            ...
            if (this.spawnTimer >= this.SPAWN_INTERVAL && this.state.enemies.size < this.MAX_ENEMIES) {
                ...
                const enemyType = enemies_json_1.default[randomEnemyTypeId];
                if (enemyType) {
                    ...
                    if (isElite) {
                        ...
                        const eliteEnemyType = enemies_json_1.default[eliteTypeId];
                        if (eliteEnemyType) {
                    ...
                    enemy.attackRange = enemyType.attackRange || 0;
                    enemy.projectileType = enemyType.projectileType || "";
            ...
            for (let i = 0; i < this.playersArray.length; i++) {
                ...
                if (player.health < player.calculatedMaxHealth && player.calculatedHealthRegen > 0) {
                    player.health = Math.min(player.calculatedMaxHealth, player.health + (player.calculatedHealthRegen * deltaTime / 1000));
                }
                ...
                if (player.attackCooldown <= 0) {
                    ...
                    if (nearestEnemy) {
                        ...
                        const projectileId = (0, uuid_1.v4)();
                        const angle = Math.atan2(targetEnemy.y - player.y, targetEnemy.x - player.x);
                        const projectile = new MyRoomState_1.Projectile();
            ...
            for (const projectile of this.projectilesArray) {
                ...
                for (const enemyId of nearbyEnemyIds) {
                    ...
                    if (!enemyPos || enemyPos.type !== 'enemy')
                        continue;
            ...
            for (let enemyIndex = 0; enemyIndex < this.enemiesArray.length; enemyIndex++) {
                ...
                const enemyTypeData = enemies_json_1.default[enemy.typeId];
                if (!enemyTypeData) {
                ...
                if (enemyTypeData.behavior === "seekAndExplode") { // Exploder behavior
                    ...
                    if (enemy.isExploding && enemy.explosionTimer > 0) {
                        ...
                        if (enemy.explosionTimer <= 0) {
                            // Deal area damage
                            const explosionRadius = enemyTypeData.explosionRadius || 120;
                            const explosionDamage = enemyTypeData.explosionDamage || 60;
                    ...
                    if (enemy.shieldCooldownTimer > 0) {
                        ...
                        const regenRate = enemyTypeData.shieldRegenRate || 5;
            ...
            for (let enemyIndex = 0; enemyIndex < this.enemiesArray.length; enemyIndex++) {
                ...
                const enemyDataForCombat = enemies_json_1.default[enemy.typeId];
                ...
                for (const playerId of nearbyPlayerIds) {
                    ...
                    if (!playerPos || playerPos.type !== 'player')
                        continue;
                    ...
                    if (distanceSquared < collisionRadiusSquared && enemy.attackCooldown <= 0) {
                        ...
                        if (!isPlayerDashing) {
                            ...
                            if (enemy.typeId === "swarm" && enemy.swarmCount > 1) {
                                const swarmBonus = enemyDataForCombat.swarmDamage || 2;
                            ...
                            if (enemy.typeId === "shield" && enemy.shieldActive) {
                                ...
                                enemy.shieldCooldownTimer = enemyDataForCombat.shieldCooldown || 3000;
                            }
            ...
            for (let playerIndex = 0; playerIndex < this.playersArray.length; playerIndex++) {
                ...
                for (const interactableId of nearbyInteractableIds) {
                    ...
                    if (!interactablePos || interactablePos.type !== 'interactable')
                        continue;
                    const interactable = this.state.interactables.get(interactableId);
                    if (!interactable || interactable.isOpen)
                        continue; // Skip already opened interactables
    ...
    onCreate(options) {
        ...
        // --- Initialize Stat Calculation Worker ---
        // Resolve path to compiled JavaScript worker file
        const workerPath = path.resolve(__dirname, './workers/statCalculator.worker.js');
        console.log(`⚙️ Initializing stat calculation worker at: ${workerPath}`);
        try {
            this.statCalculationWorker = new worker_threads_1.Worker(workerPath);
            // --- Handle Messages Received FROM Worker ---
            this.statCalculationWorker.on('message', (output) => {
                if (output.error) {
                    console.error(`❌ Worker error for player ${output.sessionId}: ${output.error}`);
                    return;
                }
                const player = this.state.players.get(output.sessionId);
                if (player && output.calculatedStats) {
                    // --- Apply calculated stats received from worker ---
                    const previousMaxHealth = player.calculatedMaxHealth;
                    player.calculatedMaxHealth = output.calculatedStats.calculatedMaxHealth;
                    player.calculatedHealthRegen = output.calculatedStats.calculatedHealthRegen;
                    player.calculatedDamage = output.calculatedStats.calculatedDamage;
                    player.calculatedAttackSpeed = output.calculatedStats.calculatedAttackSpeed;
                    player.calculatedProjectileSpeed = output.calculatedStats.calculatedProjectileSpeed;
                    player.calculatedMoveSpeed = output.calculatedStats.calculatedMoveSpeed;
                    player.calculatedArmor = output.calculatedStats.calculatedArmor;
                    player.calculatedCritChance = output.calculatedStats.calculatedCritChance;
                    // Apply health scaling if max health changed
                    if (player.calculatedMaxHealth !== previousMaxHealth) {
                        const healthRatio = player.health / previousMaxHealth;
                        player.health = Math.min(player.calculatedMaxHealth, player.health * healthRatio);
                    }
                    // Update player state
                    this.state.players.set(output.sessionId, player);
                    console.log(`⚡ Stats updated for player ${output.sessionId} from worker cache`);
                }
                else if (player) {
                    console.warn(`⚠️ Received worker message for player ${output.sessionId} but missing calculatedStats.`);
                }
            });
            this.statCalculationWorker.on('error', (err) => {
                console.error('❌ Stat calculation worker encountered an error:', err);
            });
            this.statCalculationWorker.on('exit', (code) => {
                if (code !== 0) {
                    console.error(`❌ Stat calculation worker stopped with exit code ${code}`);
                }
                else {
                    console.log('✅ Stat calculation worker exited cleanly.');
                }
            });
        }
        catch (err) {
            console.error("!!! ❌ Failed to initialize stat calculation worker !!!", err);
        }
                        ...
                        for (let i = 0; i < effects.length; i++) {
                            ...
                            if (effect.trigger === "onActivate") {
                                if (effect.effect === "dashAttack") {
                                    ...
                                    this.executeDashAttack(player, effect.damage || 50);
                                }
                                else if (effect.effect === "grenade") {
                                    // Alien Spore Pod - Throw grenade
                                    this.throwGrenade(player, effect.damage || 200, effect.radius || 150);
                                }
                            }
                        }
                        // Set equipment cooldown
                        player.attackCooldown = equipment.cooldown || 3; // 3 second default cooldown
                    }
                }
            }
        });
        ...
        this.onMessage("interact", (client, data) => {
            ...
            if (data.interactableId) {
                ...
                if (interactable) {
                    ...
                    if (distance < 60) { // Slightly increased interaction range for better UX
                        console.log(`🎯 Player ${client.sessionId} interacting with ${interactable.type} at ${data.interactableId} (distance: ${distance.toFixed(1)}px)`);
                        this.handleInteractableInteraction(player, interactable, data.interactableId);
                    }
                    else {
                        console.log(`📏 Player ${client.sessionId} too far from ${interactable.type} (${distance.toFixed(1)}px < 60px required)`);
                    }
                }
                else {
                    console.log(`❌ Interactable ${data.interactableId} not found`);
                }
            }
            else {
    ...
    handleInteractableInteraction(player, interactable, interactableId) {
    ...
    handleItemPickup(player) {
        // PERFORMANCE FIX: Use spatial grid for O(1) nearby entity detection
        const nearbyEntityIds = this.getNearbyEntities(player.x, player.y, 40); // 40px pickup range
        let pickedUpItem = false;
        for (const entityId of nearbyEntityIds) {
            // Skip if not an interactable
            const entityPos = this.entityPositions.get(entityId);
            if (!entityPos || entityPos.type !== "interactable")
                continue;
            const interactable = this.state.interactables.get(entityId);
            if (!interactable || interactable.isOpen)
                continue; // Skip already opened interactables
            // Double-check distance (spatial grid is approximate)
            const distance = Math.hypot(interactable.x - player.x, interactable.y - player.y);
            if (distance >= 40)
                continue; // Outside pickup range
            console.log(`📦 Player ${player.sessionId} picking up ${interactable.type} (spatial grid detection)`);
            switch (interactable.type) {
                case "smallChest":
                case "largeChest":
                    this.handleChestOpening(player, interactable, entityId);
                    pickedUpItem = true;
                    break;
                case "equipmentBarrel":
                    this.handleBarrelOpening(player, interactable, entityId);
                    pickedUpItem = true;
                    break;
                case "triShop":
                    this.handleTriShopInteraction(player, interactable, entityId);
                    pickedUpItem = true;
                    break;
                case "bioResonanceBeacon":
                    // Beacon should be handled via explicit interaction, not pickup
                    console.log(`🌟 Beacon detected near player ${player.sessionId}, but requires explicit interaction`);
                    break;
            }
            if (pickedUpItem)
                break; // Only pick up one item per interaction
        }
    }
    handleBeaconActivation(player, beacon, beaconId) {
        if (beacon.isOpen) {
    ...
    handleChestOpening(player, chest, chestId) {
        if (chest.isOpen) {
    ...
    handleAltarInteraction(player, altar, altarId) {
        if (altar.isOpen) {
    ...
    handleTotemInteraction(player, totem, totemId) {
        if (totem.isOpen) {
    ...
    handleBarrelOpening(player, barrel, barrelId) {
        if (barrel.isOpen) {
    ...
    handleTriShopInteraction(player, triShop, triShopId) {
        if (triShop.isOpen) {
    ...
    generateRandomItem(rarity) {
        const availableItems = Object.values(items_json_1.default).filter(item => !rarity || item.rarity === rarity);
        if (availableItems.length === 0)
            return null;
        const selectedItemData = availableItems[Math.floor(Math.random() * availableItems.length)];
        const item = new MyRoomState_1.ItemState();
        ...
        selectedItemData.effects.forEach((effectData) => {
    ...
    applyItemEffectsToPlayer(player) {
        const sessionId = player.sessionId;
        // Clear any existing timer for this player
        if (this.statCalcDebounceTimers.has(sessionId)) {
            clearTimeout(this.statCalcDebounceTimers.get(sessionId));
        }
        // Set a new timer
        const timer = setTimeout(() => {
            this.triggerWorkerStatCalculation(player);
            this.statCalcDebounceTimers.delete(sessionId); // Clean up timer map entry
        }, this.STAT_CALC_DEBOUNCE_MS);
        this.statCalcDebounceTimers.set(sessionId, timer);
    }
    // --- NEW function that actually sends job to worker ---
    triggerWorkerStatCalculation(player) {
        if (!this.statCalculationWorker || !player) {
            console.error(`❌ Cannot calculate stats for ${player === null || player === void 0 ? void 0 : player.sessionId}: Worker not initialized or player invalid!`);
            return;
        }
        // --- Prepare Data for Worker (Convert Schema Objects to Plain Data) ---
        const baseStatsData = {
        ...
        const itemsData = player.items.map(itemSchema => {
            const itemData = {
                id: itemSchema.id,
                rarity: itemSchema.rarity,
                stackingType: itemSchema.stackingType,
                effects: []
            };
            itemSchema.effects.forEach(effectSchema => {
                itemData.effects.push({
                    stat: effectSchema.stat,
                    value: effectSchema.value,
                    type: effectSchema.type,
                    trigger: effectSchema.trigger,
                    effect: effectSchema.effect,
                    interval: effectSchema.interval
                });
            });
            return itemData;
        });
        // --- Send Data TO the Worker ---
        console.log(`📤 Sending debounced stat calc job for ${player.sessionId} to worker.`);
        this.statCalculationWorker.postMessage({
            sessionId: player.sessionId,
            baseStats: baseStatsData,
            items: itemsData,
        });
    }
    calculateItemEffectsIncremental(player) {
        // PERFORMANCE FIX: Store previous stats to calculate deltas
        const previousStats = {
            maxHealth: player.calculatedMaxHealth,
            healthRegen: player.calculatedHealthRegen,
            damage: player.calculatedDamage,
            attackSpeed: player.calculatedAttackSpeed,
            projectileSpeed: player.calculatedProjectileSpeed,
            moveSpeed: player.calculatedMoveSpeed,
            armor: player.calculatedArmor,
            critChance: player.calculatedCritChance
        };
        // Reset to base stats
        ...
        // OPTIMIZED: Single pass through items with minimal calculations
        ...
        for (let itemIndex = 0; itemIndex < itemsCount; itemIndex++) {
            ...
            for (let effectIndex = 0; effectIndex < effectsCount; effectIndex++) {
                ...
                // Skip non-stat effects for performance
                if (!effect.stat || effect.stat === "healInstant")
                    continue;
                const isPercentage = effect.type === "percentage";
                const baseValue = this.getBaseStatValue(player, effect.stat);
                const effectValue = isPercentage ? baseValue * effect.value : effect.value;
                // Direct stat modification - O(1) per effect
                switch (effect.stat) {
                    case "maxHealth":
                        player.calculatedMaxHealth += effectValue;
                        break;
                    case "healthRegen":
                        player.calculatedHealthRegen += effectValue;
                        break;
                    case "damage":
                        player.calculatedDamage += effectValue;
                        break;
                    case "attackSpeed":
                        player.calculatedAttackSpeed += effectValue;
                        break;
                    case "projectileSpeed":
                        player.calculatedProjectileSpeed += effectValue;
                        break;
                    case "moveSpeed":
                        player.calculatedMoveSpeed += effectValue;
                        break;
                    case "armor":
                        player.calculatedArmor += effectValue;
                        break;
                    case "critChance":
                        player.calculatedCritChance += effectValue;
                        break;
                }
            }
        }
        // Apply soft limits (minimal calculations)
        if (player.calculatedMoveSpeed > 20) {
            const excess = player.calculatedMoveSpeed - 20;
            player.calculatedMoveSpeed = 20 + Math.sqrt(excess) * 2;
        }
        if (player.calculatedDamage > 150) {
            const excess = player.calculatedDamage - 150;
            player.calculatedDamage = 150 + Math.sqrt(excess) * 5;
        }
        if (player.calculatedAttackSpeed > 4) {
            const excess = player.calculatedAttackSpeed - 4;
            player.calculatedAttackSpeed = 4 + Math.sqrt(excess) * 0.5;
        }
        if (player.calculatedCritChance > 0.75) {
            const excess = player.calculatedCritChance - 0.75;
            player.calculatedCritChance = 0.75 + Math.sqrt(excess) * 0.1;
            player.calculatedCritChance = Math.min(player.calculatedCritChance, 0.95);
        }
        // CRITICAL: Apply health scaling if max health changed
        if (player.calculatedMaxHealth !== previousStats.maxHealth) {
            const healthRatio = player.health / previousStats.maxHealth;
            player.health = Math.min(player.calculatedMaxHealth, player.health * healthRatio);
        }
        // Update player state once (batch update)
        this.state.players.set(player.sessionId, player);
        console.log(`⚡ Incremental stat calculation completed for player ${player.sessionId}`);
    }
    getBaseStatValue(player, stat) {
        switch (stat) {
            case "maxHealth": return player.maxHealth;
            case "healthRegen": return player.healthRegen;
            case "damage": return player.damage;
            case "attackSpeed": return player.attackSpeed;
            case "projectileSpeed": return player.projectileSpeed;
            case "moveSpeed": return player.moveSpeed;
            case "armor": return player.armor;
            case "critChance": return player.critChance;
            default: return 0;
        }
    }
    calculateItemEffectsNonBlocking(player) {
    ...
    populateMap() {
        ...
        while (remainingCredits > 0 && attempts < maxAttempts) {
            ...
            if (remainingCredits >= selectedInteractable.cost) {
                ...
                const tooClose = Array.from(this.state.players.values()).some(player => Math.hypot(player.x - x, player.y - y) < minDistanceFromPlayers);
                if (!tooClose) {
                    const interactableState = new MyRoomState_1.InteractableState();
                    interactableState.id = (0, uuid_1.v4)();
    ...
    spawnBeacon() {
        ...
        const beacon = new MyRoomState_1.InteractableState();
        beacon.id = (0, uuid_1.v4)();
        ...
        // CRITICAL FIX: Add beacon to spatial grid for collision detection
        this.entityPositions.set(beacon.id, { x: beaconX, y: beaconY, type: "interactable" });
        ...
        this.log(`🔍 DEBUG: Beacon added to spatial grid at grid position (${Math.floor(beaconX / this.GRID_SIZE)}, ${Math.floor(beaconY / this.GRID_SIZE)})`);
        // Spawn Altar of the Apex near the beacon (but further away to avoid confusion)
        this.spawnAltarOfTheApex(beaconX, beaconY);
    }
    spawnAltarOfTheApex(beaconX, beaconY) {
        ...
        const altar = new MyRoomState_1.InteractableState();
        altar.id = (0, uuid_1.v4)();
        altar.type = "altarOfTheApex";
        altar.x = finalX;
        altar.y = finalY;
        altar.isOpen = false;
        this.state.interactables.set(altar.id, altar);
        // CRITICAL FIX: Add altar to spatial grid for collision detection
        this.entityPositions.set(altar.id, { x: finalX, y: finalY, type: "interactable" });
        console.log(`🗿 Altar of the Apex spawned near beacon at (${finalX.toFixed(0)}, ${finalY.toFixed(0)})`);
        console.log(`🔍 DEBUG: Altar added to spatial grid at grid position (${Math.floor(finalX / this.GRID_SIZE)}, ${Math.floor(finalY / this.GRID_SIZE)})`);
    }
    ...
    spawnBoss() {
        if (this.state.beaconState === "charging" || this.state.beaconState === "apexEmpowered") {
            ...
            const enemyType = enemies_json_1.default.charger; // Use charger as boss for now
            if (enemyType) {
                const id = (0, uuid_1.v4)();
                boss.id = id;
                ...
                if (beacon) {
                    ...
                    if (spawnX < worldBounds.minX)
                        spawnX = worldBounds.minX;
                    if (spawnX > worldBounds.maxX)
                        spawnX = worldBounds.maxX;
                    if (spawnY < worldBounds.minY)
                        spawnY = worldBounds.minY;
                    if (spawnY > worldBounds.maxY)
                        spawnY = worldBounds.maxY;
    ...
    applyProjectileEffects(projectile, hitEnemy, player) {
    ...
    applyOnHitEffect(effect, enemy, player, projectile, hitCount) {
            ...
            case "chainDamage":
                ...
                for (let chain = 0; chain < maxTargets - 1 && enemiesToHit.length > 0; chain++) {
                    ...
                    for (let i = 0; i < this.enemiesArray.length; i++) {
                        ...
                        if (hitEnemies.has(chainCandidate.id))
                            continue;
    ...
    applyOnKillEffect(effect, enemy, player, _projectile) {
        switch (effect.effect) {
            case "areaDamage":
                // Death explosion
    ...
    createAreaDamage(x, y, _player, damage, radius) {
    ...
    onJoin(client, _options) {
        ...
        const stats = characters_json_1.default.bioResearcher.stats;
    ...
    onDispose() {
        ...
        this.statCalcDebounceTimers.forEach(timer => clearTimeout(timer));
        this.statCalcDebounceTimers.clear();
        // Terminate worker
        if (this.statCalculationWorker) {
            console.log('🔄 Terminating stat calculation worker...');
            this.statCalculationWorker.terminate().then(() => {
                console.log('✅ Stat calculation worker terminated.');
            }).catch(err => {
    ...
    spawnEnemyXP(x, y, enemyType, isElite = false) {
        const xpEntity = new MyRoomState_1.XPEntityState();
        xpEntity.id = (0, uuid_1.v4)();
        xpEntity.x = x;
        xpEntity.y = y;
    ...
    spawnBossRewards(bossX, bossY) {
        ...
        if (wasApexEmpowered) {
            ...
            const bonusXp = new MyRoomState_1.XPEntityState();
            bonusXp.id = (0, uuid_1.v4)();
            bonusXp.x = bossX + 30;
            bonusXp.y = bossY + 30;
            bonusXp.xpValue = 50;
    ...
    applyDifficultyScaling(difficultyLevel) {
    ...
    applyStatusEffect(enemy, effectType, damage, duration, stacks = 1) {
        switch (effectType) {
            case "poison":
