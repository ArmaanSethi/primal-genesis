    ...
    describe("Interactable Type Validation", () => {
        it("should recognize valid interactable types", () => {
            ...
            validTypes.forEach(type => {
                assert.ok(["smallChest", "largeChest", "equipmentBarrel", "triShop"].includes(type),
                    `${type} should be a valid interactable type`);
            });
        });
        ...
        it("should reject invalid interactable types", () => {
            ...
            invalidTypes.forEach(type => {
                if (type !== null && type !== undefined) {
                    assert.ok(!["smallChest", "largeChest", "equipmentBarrel", "triShop"].includes(type),
                        `${type} should not be a valid interactable type`);
                } else {
    ...
    describe("Distance Calculations", () => {
        it("should calculate pickup distance correctly", () => {
            ...
            const distance = Math.sqrt(
                Math.pow(interactableX - playerX, 2) +
